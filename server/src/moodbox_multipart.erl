-module(moodbox_multipart).

-export([parse_multipart_request/2, test/0]).

-define(CHUNKSIZE, 4096).

-record(mp, {state, boundary, length, buffer, callback, req}).

parse_recursive(Req, Callback) ->
    {Data, {State1, State2}} = parse_multipart_request_old(Req, undefined),
    NewCallback = Callback(Data),
    case State1 of
	finished ->
	    State2;
	_ ->
	    parse_recursive(State1, State2, NewCallback)
    end.
parse_recursive(State1, State2, Callback) ->
    {Data, {NewState1, NewState2}} = feed_mp(State1, State2),
    NewCallback = Callback(Data),
    case NewState1 of
	finished ->
	    {A, B, _} = NewState2,
	    {A, B, NewCallback};
	_ ->
	    parse_recursive(NewState1, NewState2, NewCallback)
    end.

parse_multipart_request(Req, Callback) ->
    parse_recursive(Req, Callback).

parse_multipart_request_old(Req, _Callback) ->
    %% TODO: Support chunked
    Length = list_to_integer(Req:get_header_value("content-length")),
    Boundary = iolist_to_binary(get_boundary(Req:get_header_value("content-type"))),
    Prefix = <<"\r\n--", Boundary/binary>>,
    BS = size(Boundary),
    Chunk = read_chunk(Req, Length),
    Length1 = Length - size(Chunk),
    <<"--", Boundary:BS/binary, "\r\n", Rest/binary>> = Chunk,
    feed_mp(headers, #mp{boundary=Prefix, length=Length1, buffer=Rest, callback=undefined, req=Req}).
...
feed_mp(headers, State=#mp{buffer=Buffer, callback=_Callback}) ->
...
%    feed_mp(body, State1#mp{buffer=Rest, callback=NextCallback});
    {{headers, parse_headers(Headers)}, {body, State1#mp{buffer=Rest, callback=undefined}}};
feed_mp(body, State=#mp{boundary=Prefix, buffer=Buffer, callback=_Callback}) ->
...
test_parse3() ->
    ...
    Expect = [{headers,
              ...
              {body_end, <<"multipart message">>},
              {headers,
               [{"content-disposition",
                 {"form-data", [{"name", "file"}, {"filename", "test_file.txt"}]}},
                {"content-type", {"text/plain", []}}]},
              {body_eof, <<"Woo multiline text file\n\nLa la la">>}],
    ...
    Expect = [{headers,
              ...
              {headers,
               [{"content-disposition",
                 ...
                 {"content-type", {"text/plain", []}}]},
              {body_eof, <<"... contents of file1.txt ...">>}],
    TestCallback = fun (Next) -> test_callback(Next, Expect) end,
    ServerFun = fun (Socket) ->
                        case gen_tcp:send(Socket, BinContent) of
                            ok ->
